...
Memakai helper runtime `mrust_runtime::interval` sebagai polling ala-htmx `hx_poll`, lewat `iced::application(...).subscription(...)`.
...
- `Subscription::batch(vec![mrust_runtime::interval(2.0, Message::Tick)])` dipasang di `subscription` → tiap 2 detik kirim `Message::Tick`
...
- `mrust_runtime::interval(detik, Message)` → `Subscription`
